...
`Set`, functions, classes, or `unknown` aren’t valid JSON. This works pretty well.
...
a valid JSON value. How ever, what is also problem is wen the JSON date is older
than from 1970, then will go wrong.

## How to fix it

You’ll need to figure out what’s a part of your provided `Presence` type definition
